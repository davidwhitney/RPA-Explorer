...
{
    private static RpaParser ArchiveContaining(TempWorkspace workspace, string entryName, byte[] content) =>
    ...
    public void GetPreview_ImageExtension_ReturnsImageTypeWithRawBytes(string entryName)
    {
        ...
        RpaParser parser = ArchiveContaining(workspace, entryName, content);

        KeyValuePair<string, object> preview = parser.GetPreview(entryName);

        preview.Key.ShouldBe(RpaParser.PreviewTypes.Image);
    ...
    {
        using TempWorkspace workspace = new TempWorkspace();
        RpaParser parser = ArchiveContaining(workspace, entryName, Encoding.UTF8.GetBytes("hello world"));
    ...
    public void GetPreview_AudioExtension_ReturnsAudioTypeWithRawBytes(string entryName)
    {
        ...
        preview.Key.ShouldBe(RpaParser.PreviewTypes.Audio);
        preview.Value.ShouldBeOfType<byte[]>().ShouldBe(content);
    }
    ...
    public void GetPreview_VideoExtension_ReturnsVideoTypeWithRawBytes(string entryName)
    {
        ...
        preview.Key.ShouldBe(RpaParser.PreviewTypes.Video);
        preview.Value.ShouldBeOfType<byte[]>().ShouldBe(content);
    }
    ...
    public void GetPreview_UnrecognisedExtension_ReturnsUnknownTypeWithRawBytes()
    {
        ...
        RpaParser parser = ArchiveContaining(workspace, "blob.dat", content);
    ...
    {
        using TempWorkspace workspace = new TempWorkspace();
        RpaParser parser = ArchiveContaining(workspace, "a.txt", Encoding.UTF8.GetBytes("a"));
    ...
    public void GetPreview_RawRequestedForTextFile_ReturnsBytesInsteadOfString()
    {
        ...
        RpaParser parser = ArchiveContaining(workspace, "a.txt", content);
    ...
    {
        using TempWorkspace workspace = new TempWorkspace();
        RpaParser parser = ArchiveContaining(workspace, "a.txt", Encoding.UTF8.GetBytes(raw));
    ...
    {
        using TempWorkspace workspace = new TempWorkspace();
        RpaParser parser = ArchiveContaining(workspace, "code.rpyc", new byte[] { 1, 2, 3 });
